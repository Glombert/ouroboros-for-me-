# ============================
# Ouroboros — Server launcher (Ubuntu/Linux, no Google Colab)
# ============================
# Use this instead of colab_launcher.py when running on a standalone server.
# Secrets come from ~/.env (or path in OUROBOROS_ENV_FILE env var).
# Drive root is a local directory (~/ouroboros_data/ or OUROBOROS_DRIVE_ROOT).
# Repo dir defaults to ~/ouroboros_repo/ (or OUROBOROS_REPO_DIR).
...
_env_file = pathlib.Path(os.environ.get("OUROBOROS_ENV_FILE", str(pathlib.Path.home() / ".env")))
try:
    # Try python-dotenv first (more robust)
    from dotenv import load_dotenv  # type: ignore
    load_dotenv(_env_file, override=False)
    log.info(f"Loaded .env via python-dotenv: {_env_file}")
except ImportError:
...
# ----------------------------
# 0.2) Apply patch shim
# ----------------------------
# Ensure repo is in sys.path before importing ouroboros modules
_REPO_DIR_ENV = os.environ.get("OUROBOROS_REPO_DIR", str(pathlib.Path.home() / "ouroboros_repo"))
...
# ----------------------------
# 2) Local Drive root (replaces Google Drive mount)
# ----------------------------
_DRIVE_ROOT_ENV = os.environ.get("OUROBOROS_DRIVE_ROOT",
                                  str(pathlib.Path.home() / "ouroboros_data"))
DRIVE_ROOT = pathlib.Path(_DRIVE_ROOT_ENV).expanduser().resolve()
