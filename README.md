...
### v6.2.1 -- Direct API Fallback for OpenRouter Failure
- **Fix: silence when OpenRouter credits depleted** -- LLMClient.chat() now falls back to direct provider APIs (Anthropic → DeepSeek → Google) when OpenRouter raises any exception (402, credits, timeout).
- **Model name mapping** -- OpenRouter model IDs (e.g. `anthropic/claude-sonnet-4.6`) automatically mapped to native provider IDs (`claude-sonnet-4-5`, `deepseek-chat`, `gemini-2.0-flash`).
- **Fix: OUROBOROS_MODEL_LIGHT** -- default was `google/gemini-3-pro-preview` (non-existent), changed to `google/gemini-2.0-flash`.
